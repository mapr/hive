...
public final class MapRDBSerDeUtils {
  ...
  private static Object deserializePrimitive(Object value, PrimitiveTypeInfo valueTypeInfo) {
    ...
    case TIMESTAMP:
      return Timestamp.ofEpochMilli(((Value) value).getTimestampAsLong());
    case BYTE: // tinyint
  ...
  private static Object serializeField(Object o, ObjectInspector oi, Map<String, String> mappings) {
    ...
    case PRIMITIVE: {
      ...
      case DATE: {
        DateWritableV2 dw = ((DateObjectInspector) poi).getPrimitiveWritableObject(o);
        result = new ODate(dw.get().getYear(), dw.get().getMonth(), dw.get().getDay());
        break;
      }
      case TIMESTAMP: {
        TimestampWritableV2 tsw = ((TimestampObjectInspector) poi).getPrimitiveWritableObject(o);
        result = new OTimestamp(tsw.getTimestamp().toEpochMilli());
        break;
      }
      default:
