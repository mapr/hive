...
public class HiveConnection implements java.sql.Connection {
  ...
  private TTransport createUnderlyingTransport() throws TTransportException {
    ...
    if (isSslConnection()) {
      ...
      transport = HiveAuthUtils.getSSLSocket(host, port, loginTimeout, sslTrustStore, sslTrustStorePassword, sslProtocolVersion);
    } else {
      ...
      if (!JdbcConnectionParams.AUTH_SIMPLE.equals(sessConfMap.get(JdbcConnectionParams.AUTH_TYPE))) {
        ...
        if (JdbcConnectionParams.AUTH_MAPRSASL.equalsIgnoreCase(sessConfMap.get(JdbcConnectionParams.AUTH_TYPE))) {
          if (JdbcConnectionParams.TRUE.equalsIgnoreCase(sessConfMap.get(JdbcConnectionParams.USE_SSL))) {
            transport = MapRSecSaslHelper.getTransport(HiveAuthUtils.getTrustAllSSLSocket(host, port, loginTimeout), saslProps);
          } else {
